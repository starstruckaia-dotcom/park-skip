...
export default function Onboard() {
  ...
  const next = () => {
    setError("");
    if (step === 0 && (!form.owner_name || !form.email)) { setError("Please fill in your name and email."); return; }
    ...
    <div style={S.page}>
      <div style={S.container}>
        ...
        {!done ? (<>
          ...
          <div style={S.card}>
            {step===0&&<><h2 style={S.cardTitle}>YOUR INFORMATION</h2><p style={S.cardSub}>Tell us about yourself</p>{[["owner_name","Full Name","Jane Smith"],["email","Email Address","jane@example.com","email"],["phone","Phone (optional)","+1 613 555 0000","tel"]].map(([f,l,p,t="text"])=><div key={f} style={S.field}><label style={S.label}>{l}</label><input style={S.input} type={t} value={form[f]} onChange={e=>update(f,e.target.value)} placeholder={p}/></div>)}</>}
